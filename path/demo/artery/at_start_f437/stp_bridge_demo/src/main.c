...
//stp_config.h 
#define  SET_STP_MODE_ENABLE           STP_SUPPORT
#define  STP_FUNCTION_ENABLE           (RSTP_SUPPORT | STP_SUPPORT)

/* add stp and rstp_start */
#if SET_STP_MODE_ENABLE
#include "stp/stp.h"
...
#if SET_STP_MODE_ENABLE
//Application configuration
#define STP_BRIDGE_NUM_PORTS DM_SWITCH_NUM_PORTS
...
#else
//Application configuration
#define RSTP_BRIDGE_NUM_PORTS DM_SWITCH_NUM_PORTS
...
/* edit stp and rstp_start */
#if SET_STP_MODE_ENABLE	 
   TRACE_INFO("*** CycloneTCP STP Bridge Demo  ***\r\n");
#else
   TRACE_INFO("*** CycloneTCP RSTP Bridge Demo ***\r\n");
#endif	 
/* edit stp and rstp_end */
   TRACE_INFO("***********************************\r\n");
...
/* add stp and rstp_end */
   ...
   netSetSwitchDriver(interface, &dm8x06SwitchDriver);	 
...
{
   uint8_t port;	
   for(port= 1; port <= stpBridgeContext.numPorts; port++)
   {
      stpSetAdminPortState(&stpBridgeContext, port, TRUE);
   }
}	 
   ...
   //Attach the STP bridge context to the BRIDGE-MIB and RSTP-MIB
...
{
   uint8_t port;	
   for(port= 1; port <= rstpBridgeContext.numPorts; port++)
   {
      rstpSetAdminPointToPointMac(&rstpBridgeContext, port, RSTP_ADMIN_P2P_MAC_FORCE_TRUE);
      rstpSetAutoEdgePort(&rstpBridgeContext, port, TRUE);
      rstpSetAdminPortState(&rstpBridgeContext, port, TRUE);
   }
 }	 
