...
error_t dm8x06Init(NetInterface *interface)
{
   ...
   TRACE_INFO("ETH_PORT_TAGGING_SUPPORT == ENABLED ....\r\n");
	 ...
	 dm8x06WriteSwitchAbsoluteReg(interface, DM8x06_SPECIAL_TAG_ETHER_TYPE, 
				DM8x06_SPECIAL_TAG_ETHER_TYPE_DEFAULT);	 
   ...
   //Disable special tag mode
	 dm8x06WriteSwitchAbsoluteReg(interface, DM8x06_CPU_PORT_AND_MIRROR_CONTROL, 
				DM8x06_CPU_PORT_SET(DM8x06_CPU_PORT));
#endif	 
...
/*				
         SwitchFdbEntry entry;
				 ...
				 entry.macAddr.w[0] = 0x0204;
				 entry.macAddr.w[1] = 0x0000;
				 entry.macAddr.w[2] = 0x1234;
				 entry.destPorts = 3;
				
				 dm8x06AddStaticFdbEntry(interface, &entry);

				 entry.macAddr.w[0] = 0x0204;
				 entry.macAddr.w[1] = 0x0001;
				 entry.macAddr.w[2] = 0xabcd;
				 entry.destPorts = 3;
...
void dm8x06EnableRsvdMcastTable(NetInterface *interface, bool_t enable)
{
  ...
  //01-80-C2-00-00-0F
	if(enable)
	{
    dm8x06WriteSwitchAbsoluteReg(interface, DM8x06_SPEIAL_PACKET_CONTROL_0, (2 << 11) | (1 << 8) | (2 << 5) | (1 << 0));  // 01 , 00
    dm8x06WriteSwitchAbsoluteReg(interface, DM8x06_SPEIAL_PACKET_CONTROL_1, (2 << 11) | (1 << 8) | (2 << 5) | (1 << 0));  // 03 , 02
    dm8x06WriteSwitchAbsoluteReg(interface, DM8x06_SPEIAL_PACKET_CONTROL_2, (2 << 11) | (1 << 8) | (2 << 5) | (1 << 0));  // 0e , 04~0d & 0f
   ...
   const SwitchFdbEntry *entry)
{
   ...
   if(macIsMulticastAddr(&entry->macAddr))
   {
     ...
     dm8x06WriteSwitchAbsoluteReg(interface, DM8x06_ADDRESS_TABLE_CONTROL_AND_STATUS, 
       DM8x06_ATB_CMD_WRITE | DM8X06_ATB_INDEX_MULTICAST);			 
   ...
   const SwitchFdbEntry *entry)
{
  ...
  dm8x06WriteSwitchAbsoluteReg(interface, DM8x06_ADDRESS_TABLE_DATA_1, htons(entry->macAddr.w[2]));
	...
	//search cmd 
  dm8x06WriteSwitchAbsoluteReg(interface, DM8x06_ADDRESS_TABLE_CONTROL_AND_STATUS, 
    DM8x06_ATB_CMD_SEARCH | DM8X06_ATB_INDEX_MAC);	
  ...
  else
  {
      ...
      if(macIsMulticastAddr(&entry->macAddr))
        ...
        //delete cmd
        dm8x06WriteSwitchAbsoluteReg(interface, DM8x06_ADDRESS_TABLE_CONTROL_AND_STATUS, 
          DM8x06_ATB_CMD_DELETE | DM8X06_ATB_INDEX_MULTICAST);
   ...
   SwitchFdbEntry *entry)
{
  ...
  if(index < DM8x06_MAC_TABLE_SIZE)
  {
    ...
    dm8x06WriteSwitchAbsoluteReg(interface, DM8x06_ADDRESS_TABLE_INDEX_SET, index);
 		...
 		//read cmd
    dm8x06WriteSwitchAbsoluteReg(interface, DM8x06_ADDRESS_TABLE_CONTROL_AND_STATUS, 
      DM8x06_ATB_CMD_READ | DM8X06_ATB_INDEX_MAC);	
...
void dm8x06FlushStaticFdbTable(NetInterface *interface)
{
     ...
     for(port = DM8x06_PORT0; port <= DM8x06_CPU_PORT; port++)
       ...
       dm8x06_ATP_Wait_busy(interface);
		   ...
		   //clean cmd 
       dm8x06WriteSwitchAbsoluteReg(interface, DM8x06_ADDRESS_TABLE_CONTROL_AND_STATUS, 
         DM8X06_ATB_CMD_CLEAN | DM8X06_ATB_INDEX_UNICAST);	
       ...
       dm8x06_ATP_Wait_busy(interface);
		   ...
		   //clean cmd 
       dm8x06WriteSwitchAbsoluteReg(interface, DM8x06_ADDRESS_TABLE_CONTROL_AND_STATUS, 
         DM8X06_ATB_CMD_CLEAN | DM8X06_ATB_INDEX_MULTICAST);	 
	   }		 
#endif		 
...
void dm8x06DumpPhyReg(NetInterface *interface, uint8_t port)
{
   ...
   for(i = 0; i < 6; i++)
   {
      ...
      TRACE_DEBUG("%02" PRIu8 ": 0x%04" PRIX16 "\r\n", i,
         dm8x06ReadPhyReg(interface, port, i));
   }
...
void dm8x06WriteSwitchAbsoluteReg(NetInterface *interface, uint16_t address,
   uint16_t data)
{
